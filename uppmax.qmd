...
In order to log in to the cluster, you need to apply for an account. You can register a new account [here](https://supr.naiss.se/login/).
You will also use this page to log on when registering projects.

## Appyling for projects
For an extensive guide of how to start with UPPMAX have a look [here](https://www.uppmax.uu.se/support/getting-started/). 
...
When you have [logged in](https://supr.naiss.se/login/), there will be a list of options in the panel on the left, select 'Proposals' to see your [proposals](https://supr.naiss.se/proposal/), scroll down to 'Rounds and 'Resources', and click on 'Go to Rounds' to get to the [application](https://supr.naiss.se/round/?).
...
Please see our [guide](https://www.slubi.se/learning-resources.html) or a more [comprehensive version](https://www.uppmax.uu.se/support/user-guides/guide--first-login-to-uppmax/)
...
* If you want to see the modules that match your search term ```module spider tool_name```. This is not case specific, so you could search "fastqc" to see all modules that match the string "fastqc" (without the inverted commas!)
* To load a module ```module load tool_name``` from the ```module spider``` results. This is case specific, so be sure to include version numbers, capital letters, and any punctuation present.
* A good habit to get into is to run ```module load bioinfo-tools``` the moment you log on to ensure that all modules contained within ```bioinfo-tools``` is available to load
* You can load multiple modules at once by simply listing them, e.g.: ```module load bioinfo-tools blast/2.14.1+```


For a list of installed software, please see [here](https://www.uppmax.uu.se/resources/software/installed-software/). 
...
##  Slurm
UPPMAX uses a job manager called [Slurm](https://slurm.schedmd.com/documentation.html). Please be sure to read their [guide](https://www.uppmax.uu.se/support/user-guides/slurm-user-guide/) before you start working on UPPMAX! This is simply a short overview of the most commonly used functions by SLUBI, and not an exhaustive list.

To use Slurm, your project number from the **compute** project is used for the **-A** flag. Your storage project number is just the place you store data. Here, the / and spaces in the name are replaced by - as you can see below
